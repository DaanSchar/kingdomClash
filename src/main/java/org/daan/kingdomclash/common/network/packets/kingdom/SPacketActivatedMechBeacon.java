...
public class SPacketActivatedMechBeacon {

    private final BlockPos pos;

    public SPacketActivatedMechBeacon(BlockPos pos) {
        this.pos = pos;
    }

    public SPacketActivatedMechBeacon(FriendlyByteBuf buf) {
        ...
        context.enqueueWork(() -> DistExecutor.unsafeCallWhenOn(Dist.CLIENT, () -> () -> {
            ClientKingdomData.getPlayer().ifPresent(
                    player -> {
                        ...
                        player.playSound(SoundEvents.BEACON_ACTIVATE, 1f, 1f);
                    }
            );
