...
public class SPacketExplosion {
    ...
    private final String kingdomUnderAttack;

    public SPacketExplosion(String kingdomUnderAttack, BlockPos crystalPos) {
        this.crystalPos = crystalPos;
        this.kingdomUnderAttack = kingdomUnderAttack;
    }

    public SPacketExplosion(FriendlyByteBuf buf) {
        this.crystalPos = buf.readBlockPos();
        this.kingdomUnderAttack = buf.readUtf();
    }

    public void toBytes(FriendlyByteBuf buf) {
        buf.writeBlockPos(this.crystalPos);
        buf.writeUtf(this.kingdomUnderAttack);
    }
    ...
    public boolean handle(Supplier<NetworkEvent.Context> supplier) {
        ...
        context.enqueueWork(() -> DistExecutor.unsafeCallWhenOn(Dist.CLIENT, () -> () -> {
            ClientKingdomData.getPlayer().ifPresent(
                    player -> spawnSoulParticles(player.level, this.crystalPos)
            );
